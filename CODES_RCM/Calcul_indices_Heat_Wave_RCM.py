...
for year in range(yi,yf+1):
    ...
    res.append(HWDI_2(DS_date_range,ind1,ind2,ind3,3))
    
print("result -concatenation-")    
res = np.stack(res)
print("result -writing Netcdf-")      
data_set = xr.Dataset( coords={'lon': (['y', 'x'], DS.lon),
                                 'lat': (['y', 'x'], DS.lat),
                                 'time': pd.date_range(str(yi)+'-01-01', periods=len(res), freq='Y')})
    
data_set["Heat Wave"] = (['time','y', 'x'],  res)
    
data_set.to_netcdf('./'+model+'_'+str(ind1)+'_'+str(ind2)+'_'+str(ind3)+'_'+str(yi)+'_'+str(yf)+'.nc')
